...
The decomposition can be obtained by substituting $U_A = Z U_\Sigma$ for a Cholesky factor $Z$ into \Cref{cholwishartdensity}, yielding the following result:
\begin{theorem}[{\cite[Theorem 3.2.14]{muirhead_aspects_1982}}]\label{cor:wishartbartlett}
    Let $U_A = Z U_\Sigma$.
    Then
    \begin{equation}
...
Namely, we make the substitution $U_B = Z^{-1} U_\Omega$ for a Cholesky factor $Z$ into \Cref{cholinvwishartdensity}.
...
Proving this result requires \Cref{thm:triinvjac} and \Cref{lem:trimuldetjac}.
...
\begin{corollary}
    ...
    Then \Cref{thm:triinvjac} and \Cref{lem:trimuldetjac} imply $\dmeasure{U_B} \propto \det{Z}^{-m-1} \dmeasure{Z}$.
\end{corollary}

\begin{proof}[\textbf{Proof of \Cref{thm:invwishartbartlettdensity}}]
    First we note that $(u_\Omega)_{jj} = z_{jj} (u_B)_{jj}$.
    Then we use the change of variables formula with the density of \Cref{cholinvwishartdensity} and Jacobian determinant of \Cref{invwishartbartlett}:
    \begin{align*}
...
\Cref{cor:invwishartbartlett} motivates \Cref{alg:rinvwishartchol} for generating a random inverse-Wishart Cholesky factor.
We then use \Cref{alg:rinvwishartchol} in \Cref{alg:rinvwishartdirect} for generating random inverse-Wishart matrices.
...
If the scale matrix is specified as $\Sigma$ or $U_\Sigma$, then \Cref{alg:rinvwishartindirect} is the most efficient, while \Cref{alg:rinvwishartdirect} is more efficient if the scale matrix is specified as $\Omega$ or $U_\Omega$.
